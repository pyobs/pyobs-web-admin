...
# otherwise share (see ejabberd-integration.md, Data layer). The HTTP API returns the same
...
def _use_http() -> bool:
    ...
    (see ejabberd-integration.md) -- not a fallback for any HTTP failure, which should
...
def stats(name: str) -> int:
    ...
    ejabberd-integration.md, Data layer)."""
    if _use_http():
...
def check_account(user: str) -> bool:
    ...
    ejabberdctl signal this two different ways (see ejabberd-integration.md, Data layer):
...
#
# See ejabberd-user-management.md, Design "Transport": a write's cost is dominated by a
...
def ban_account(user: str, reason: str) -> None:
    ...
    ejabberd-user-management.md's Current state). Note check_password must never be used to
...
def get_ban_details(user: str) -> dict | None:
    ...
    returning a clean answer (verified live, see ejabberd-user-management.md's "Verified
...
def kick_session(user: str, resource: str, reason: str) -> None:
    ...
    always "pyobs" for a pyobs module -- confirmed live, see ejabberd-integration.md's Data
