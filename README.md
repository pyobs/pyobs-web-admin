...
- **Module detail** — per-module view with four tabs:
  ...
  - *Logs* — live log tail with text filter, time-range filter (click a line to set), colour-coded by severity, auto-refresh; scrolling to the top auto-loads older entries (journald-backed modules only, see [DEV_JOURNALD_LOGS.md](DEV_JOURNALD_LOGS.md))
