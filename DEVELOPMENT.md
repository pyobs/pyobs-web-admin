...
- [ACL_MATRIX.md](ACL_MATRIX.md) — fleet-wide ACL matrix page: view every module's `acl:`
  ...
  (named, reusable caller-lists): storage layer, a standalone `/groups/` management page
  (create/edit/delete), and expand-on-save (assigning a group to a target's `allow`/`deny` in
  either ACL editing surface now expands it into literal caller entries, with the applied
  snapshot recorded for later) are all shipped. **Re-apply is not built yet** — nothing reads
  that recorded snapshot back to show drift or offer a resync when a group's membership
