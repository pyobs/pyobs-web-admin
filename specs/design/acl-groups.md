...
lost if Groups comes up again later. `acl-matrix.md` itself has no remaining trace of this
...
Raised after `ejabberd-integration.md` shipped: ejabberd's `mod_shared_roster` has its own
...
this way, and `acl-matrix.md`'s own Design section is explicit that "callers are just
