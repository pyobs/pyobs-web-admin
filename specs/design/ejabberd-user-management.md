...
instances the way `ejabberd-integration.md`'s reads were).

## Motivation

`ejabberd-integration.md` gave this app read-only visibility into ejabberd (who's registered,
...
- `modules/ejabberd.py` has one function per **read** command only
  ...
  and an `ejabberdctl` subprocess fallback (~0.5–0.6s/call) — see `ejabberd-integration.md`,
...
- ejabberd's own `api_permissions` config (the loopback-only `mod_http_api` grant
  `ejabberd-integration.md` documents and verified live) whitelists exactly seven read commands
...
residue. This directly follows `ejabberd-integration.md`'s own precedent (its "Credential layer
...
- **A real fact this doc's config-write-back decision depends on:** `ejabberd-integration.md`'s
...
fewer deploy-time step for this feature than `ejabberd-integration.md` needed.
...
(reusing `get_comm_user` across `list_modules()`, the same way `ejabberd-integration.md`'s own
...
The module detail page's existing ejabberd block (Overview tab, `ejabberd-integration.md`'s
...
scope than "manage an existing identity." Matches `ejabberd-integration.md`'s own read-only
...
- [x] Hub-mode delegation for the write actions, mirroring the existing read delegation
  ...
  hub/spoke pair the way the read path was in `ejabberd-integration.md`. Worth doing before
...
- [x] `README.md`: document once implemented and verified live, not before — matches this
  ...
  (see `ejabberd-integration.md`'s Progress log, and `journald-logs.md`'s Work Plan, for the
